    ...
    <div className="relative border-l-2 border-border-color pl-6 sm:pl-8 space-y-12">
      {data.awards.map((award) => (
        <div key={award.id} className="relative">
          ...
          <div className="mt-2 text-text-secondary prose" dangerouslySetInnerHTML={{ __html: award.description }} />
          {award.attachmentUrl && (
            <a
              href={award.attachmentUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center text-primary hover:underline text-sm mt-3"
            >
