...
const ExperienceItem: React.FC<{ job: any; index: number }> = ({ job, index }) => (
  <div key={job.id} className="relative">
    ...
    <div className="mt-2 text-text-secondary prose" dangerouslySetInnerHTML={{ __html: job.description }} />
    <div className="mt-3 flex flex-wrap gap-2">
      {job.skillsUsed.map((skill) => (
        <span key={skill} className="bg-primary/10 text-primary text-xs font-semibold px-2.5 py-0.5 rounded-full">
          {skill}
        </span>
      ))}
    </div>
      ...
      {virtualizationConfig.enabled ? (
        ...
        <div className="relative">
          <VirtualizedList
            ...
            renderItem={(job, index) => <ExperienceItem job={job} index={index} />}
          />
        </div>
      ) : (
