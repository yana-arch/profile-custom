  ...
  return (
    <div key={project.id} className={cardClasses}>
      ...
      <div className="p-6">
        ...
        <div className="text-text-secondary mb-4 prose" dangerouslySetInnerHTML={{ __html: project.description }} />
    ...
    >
      {virtualizationConfig.enabled ? (
        ...
        <VirtualizedList
          ...
          renderItem={(project) => <ProjectCard project={project} hoverEffect={hoverEffect} viewMode={viewMode} />}
        />
      ) : (
        // Regular grid view for fewer projects
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {data.projects.map((project) => (
            <ProjectCard key={project.id} project={project} hoverEffect={hoverEffect} viewMode={viewMode} />
          ))}
        </div>
      )}
    </AnimatedProfileSection>
