...
export const ProfileSection: React.FC<{ title: string; id: string; children: React.ReactNode }> = ({
  title,
  id,
  children,
}) => (
  <section id={id} className="py-16 md:py-24 px-4 sm:px-6 lg:px-8 w-full">
    <div className="max-w-5xl mx-auto">
      <h2 className="text-2xl md:text-3xl font-bold text-text-primary mb-12 border-b-2 border-primary pb-2 inline-block">
        {title}
      </h2>
      {children}
    </div>
...
const AnimatedProfileSection: React.FC<AnimatedProfileSectionProps> = ({
  title,
  id,
  children,
  scrollAnimation,
