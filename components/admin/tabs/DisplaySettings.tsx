    ...
    <AdminSection title="Display Settings">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            {['scroll', 'tab', 'slide'].map((layout) => (
              <label key={layout} className="flex items-center">
                <input
                  ...
                  name="layout"
                  value={layout}
                  checked={data.settings.layout === layout}
        ...
        <div className="flex items-center space-x-4">
          <label htmlFor="primaryColor" className="font-semibold text-text-primary">
            Primary Color
          </label>
          <input
            type="color"
        ...
        <div className="flex items-center space-x-4">
          <label htmlFor="secondaryColor" className="font-semibold text-text-primary">
            Secondary Color
          </label>
          <input
            type="color"
        ...
        <div className="col-span-1 md:col-span-2 border-t border-border-color pt-4 mt-2">
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="borderRadius" className="block text-sm font-medium text-text-secondary mb-1">
                Border Radius ({data.settings.borderRadius}px)
              </label>
              <input
                type="range"
                name="borderRadius"
            ...
            <SelectField
              label="Box Shadow Strength"
              name="boxShadowStrength"
              value={data.settings.boxShadowStrength}
              onChange={handleSettingsChange}
            >
            ...
            <div>
              <label htmlFor="transitionDuration" className="block text-sm font-medium text-text-secondary mb-1">
                Transition Duration ({data.settings.transitionDuration}ms)
              </label>
              <input
                type="range"
                ...
                max="1000"
                step="25"
                value={data.settings.transitionDuration}
                onChange={handleSettingsChange}
                className="w-full"
              />
            </div>
            ...
            {Object.keys(data.settings.sections).map((sectionKey) => (
              <label key={sectionKey} className="flex items-center">
                <input
                  ...
                  checked={data.settings.sections[sectionKey as keyof typeof data.settings.sections]}
                  onChange={handleSettingsChange}
      ...
      <div className="mt-8 border-t border-border-color pt-6">
        ...
        <div className="flex space-x-4">
          <button onClick={handleExport} className="bg-secondary/80 text-white px-4 py-2 rounded-md hover:bg-secondary">
            Export Data
          </button>
          <button
            onClick={handleImportClick}
            className="bg-border-color text-text-secondary px-4 py-2 rounded-md hover:bg-primary/20"
          >
            Import Data
          </button>
