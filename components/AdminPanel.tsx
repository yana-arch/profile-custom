...
const AdminPanel: React.FC<AdminPanelProps> = ({ data, setData, initialTab }) => {
  ...
  const contentTabs = [
    {
      ...
      name: 'Personal Info',
      icon: UserIcon,
      component: <PersonalInfoSettings data={data} setData={setData} />,
    },
    {
      id: 'experience',
      name: 'Experience',
      icon: BriefcaseIcon,
      component: <ExperienceSettings data={data} setData={setData} />,
    },
    {
      id: 'education',
      name: 'Education',
      icon: AcademicCapIcon,
      component: <EducationSettings data={data} setData={setData} />,
    },
    {
      id: 'projects',
      name: 'Projects',
      icon: CodeBracketIcon,
      component: <ProjectsSettings data={data} setData={setData} />,
    },
    {
      id: 'skills',
      name: 'Skills',
      icon: WrenchScrewdriverIcon,
      component: <SkillsSettings data={data} setData={setData} />,
    },
    {
      id: 'certifications',
      name: 'Certifications',
      icon: CertificateIcon,
      component: <CertificationsSettings data={data} setData={setData} />,
    },
  ...
  const appearanceTabs = [
    {
      ...
      name: 'Display',
      icon: PaintBrushIcon,
      component: <DisplaySettings data={data} setData={setData} />,
    },
    {
      id: 'animations',
      name: 'Animations',
      icon: EyeIcon,
      component: <AnimationSettings data={data} setData={setData} />,
    },
  ];
  ...
  const AccordionSection: React.FC<{
    ...
    <div>
      ...
      >
        <span>{title}</span>
        <ChevronDownIcon
          className={`w-5 h-5 text-text-secondary/80 transition-transform ${openSection === sectionName ? 'rotate-180' : ''}`}
        />
      </button>
        ...
        <div className="space-y-1 mt-2 animate-fadeIn">
          {tabs.map((tab) => (
            <TabButton
              ...
              setTab={() => handleTabClick(tab.id, sectionName)}
            />
          ))}
        </div>
      )}
  ...
  return (
    <div className="flex h-screen bg-background text-text-primary">
      {/* Admin UI Container */}
      <div
        className={`flex flex-col h-full overflow-hidden transition-all duration-300 ease-in-out ${showPreview ? 'w-full lg:w-[55%]' : 'w-full'}`}
      >
      ...
      {/* Live Preview Pane */}
      <div
        className={`h-full transition-all duration-300 ease-in-out overflow-hidden hidden lg:block ${showPreview ? 'w-[45%]' : 'w-0'}`}
      >
