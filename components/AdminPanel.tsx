...
import {
  ...
  CertificateIcon, TrophyIcon, HeartIcon, CogIcon, SparklesIcon, EyeIcon, PaintBrushIcon
} from './icons/Icons';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ data, setData, setIsViewOnly, initialTab }) => {
  ...
  const TabButton: React.FC<{tab: {id: string; name: string; icon: React.ElementType}, currentTab: string, setTab: (id: string) => void}> = ({tab, currentTab, setTab}) => {
    const Icon = tab.icon;
    return (
       <button
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-background">
      <aside className="w-full md:w-64 bg-card-background border-r border-border-color p-4 flex-shrink-0">
        <h2 className="text-xl font-bold text-text-primary mb-6 px-2">Admin Panel</h2>
        <nav className="space-y-4">
            <div>
                <h3 className="px-4 text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2">Content</h3>
                <div className="space-y-1">
                    {contentTabs.map(tab => <TabButton key={tab.id} tab={tab} currentTab={activeTab} setTab={setActiveTab} />)}
                </div>
            </div>
            <div>
                <h3 className="px-4 text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2 mt-4">Appearance</h3>
                <div className="space-y-1">
                    {appearanceTabs.map(tab => <TabButton key={tab.id} tab={tab} currentTab={activeTab} setTab={setActiveTab} />)}
                </div>
            </div>
            <div>
                <h3 className="px-4 text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2 mt-4">AI Tools</h3>
                <div className="space-y-1">
                    {aiTabs.map(tab => <TabButton key={tab.id} tab={tab} currentTab={activeTab} setTab={setActiveTab} />)}
                </div>
            </div>
        </nav>
      </aside>
