-- Comprehensive Row Level Security Setup for All Tables (FIXED VERSION)
-- Run this in Supabase SQL Editor after running initial migrations
-- This version fixes the user creation issues with triggers
...
-- Users table permissions - allow anon inserts for triggers
...
-- Allow anon inserts for user creation triggers
DROP POLICY IF EXISTS "Allow anon inserts for user creation" ON users;
CREATE POLICY "Allow anon inserts for user creation" ON users FOR INSERT WITH CHECK (true);
...
-- Users can delete their own profile (if needed)
...
-- Anonymous users can view non-premium templates
DROP POLICY IF EXISTS "Anon can view free templates" ON templates;
CREATE POLICY "Anon can view free templates" ON templates FOR SELECT
USING (auth.role() = 'anon' AND NOT is_premium);
...
-- =====================================================
-- FIX USER SYNCHRONIZATION (RECREATE TRIGGERS)
-- =====================================================
...
-- RLS is now comprehensively configured for all tables with trigger fixes
